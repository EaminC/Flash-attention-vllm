...
Usage example:
   python flash_attention_benchmark.py --iters 100 --spread_min 1 --spread_max 10 --spread_step 1
"""
...
def main():
    ...
    parser.add_argument("--iters", type=int, default=100, help="Number of timed iterations (excluding warmup)")
    parser.add_argument("--spread_min", type=int, default=1, help="Minimal spread parameter value")
    parser.add_argument("--spread_max", type=int, default=151, help="Maximal spread parameter value")
    parser.add_argument("--spread_step", type=int, default=1, help="Step size for the spread parameter")
    args = parser.parse_args()

    spreads = list(range(args.spread_min, args.spread_max + 1, args.spread_step))

    # Define two sets of configuration parameters
    ...
    # For each configuration, run contiguous mode tests 
    # (only once because contiguous mode results are independent of spread)
    print("Running contiguous benchmark for Basic configuration...")
    contiguous_results_basic, _ = run_one_benchmark_config(basic_config, continuous=True, spread=1, num_iters=args.iters)
    
    print("Running contiguous benchmark for BERT Base configuration...")
    contiguous_results_bert, _ = run_one_benchmark_config(bert_config, continuous=True, spread=1, num_iters=args.iters)
    
    # For each configuration, run non-contiguous mode tests across different spreads
    print("Running non-contiguous benchmarks for Basic configuration...")
    noncontiguous_results_basic = collect_benchmarks_for_config(basic_config, spreads, num_iters=args.iters)
    
    print("Running non-contiguous benchmarks for BERT Base configuration...")
    noncontiguous_results_bert = collect_benchmarks_for_config(bert_config, spreads, num_iters=args.iters)
    
    # Plot the results
    print("Plotting results for Basic configuration...")
    plot_results_for_config("Basic", spreads, contiguous_results_basic, noncontiguous_results_basic)

    print("Plotting results for BERT Base configuration...")
    plot_results_for_config("BERT Base", spreads, contiguous_results_bert, noncontiguous_results_bert)

    print("Benchmark complete, figures have been saved.")
